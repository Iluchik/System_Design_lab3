...
from services.package_service import package_service, Package, Package_description
from model.model import User_description, User_response, get_db, Package_description, Package_response
...
@app.post("/package", response_model=Package_response)
async def create_package(package_desc: Package_description, current_user: dict = Depends(user_service.authentification), db:Session = Depends(get_db), tags=["packages"]):
	try:
		response: Package_response = await package_service.create_package(package_desc=package_desc, current_user=current_user, db=db)
		return response
	except Exception as e:
		raise e

@app.get("/package", response_model=List[Package_response], tags=["packages"])
async def get_user_packages(current_user: dict = Depends(user_service.authentification), db:Session = Depends(get_db)):
	try:
		response: List[Package_response] = await package_service.get_user_packages(current_user=current_user, db=db)
		return response
	except Exception as e:
		raise e

@app.put("/package", response_model=Package_response)
async def update_package(updated_package: dict, current_user: dict = Depends(user_service.authentification), db:Session = Depends(get_db), tags=["users"]):
	try:
		response: Package_response = await package_service.update_package(updated_package=updated_package, current_user=current_user, db=db)
		return response
	except Exception as e:
		raise e


@app.delete("/package/{product_id}", response_model=Package_response, tags=["users"])
async def delete_package(product_id: str, current_user: dict = Depends(user_service.authentification), db:Session = Depends(get_db)):
	try:
		response: Package_response = await package_service.delete_package(product_id=product_id, current_user=current_user, db=db)
		return response
	except Exception as e:
		raise e
